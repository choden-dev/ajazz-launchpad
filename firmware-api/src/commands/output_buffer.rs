...
/// * `lsb_buffer` - byte array to overwrite the lower bytes (will contain your intended message)
pub fn create_output_buffer_with_lsbs_filled<const N: usize>(lsb_buffer: &[u8]) -> ByteArray<N> {
    let mut buffer = [0u8; N];
    let length = lsb_buffer.len().min(N);

    buffer[..length].copy_from_slice(&lsb_buffer[..length]);
    buffer
}
...
mod tests {
    ...
    fn fills_in_lower_bytes_happy_path() {
        let output = create_output_buffer_with_lsbs_filled::<513>(&[0x09, 0x90]);
    ...
    fn truncates_oversized_message() {
        ...
        let output = create_output_buffer_with_lsbs_filled::<10>(&message);
